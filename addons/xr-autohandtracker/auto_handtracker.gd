...
func extractrestfingerbones():
	# Define o prefixo com base na mão (0 = Esquerda, 1 = Direita)
	var prefix = "Left" if hand == 0 else "Right"
	
	handtoskeltransform = handnode.global_transform.inverse() * skel.global_transform
	
	# O osso do pulso no seu modelo se chama "LeftHand" ou "RightHand"
	var wrist_name = prefix + "Hand"
	wristboneindex = skel.find_bone(wrist_name)
	
	if wristboneindex == -1:
		#print("❌ ERRO GRAVE: Não achou o osso do pulso: ", wrist_name)
		return
	...
	for f in ["Thumb", "Index", "Middle", "Ring", "Little"]:
		...
		for b in ["Metacarpal", "Proximal", "Intermediate", "Distal", "Tip"]:
			# O dedão não tem o osso 'Intermediate', então nós ignoramos ele para não dar erro
			if f == "Thumb" and b == "Intermediate":
				continue
				
			# Monta o nome exato, ex: "Right" + "Index" + "Tip" = "RightIndexTip"
			var bone_name = prefix + f + b
			var ix = skel.find_bone(bone_name)
			
			if ix != -1:
				fingerboneindexes[-1].push_back(ix)
				fingerboneresttransforms[-1].push_back(skel.get_bone_rest(ix))
			else:
				print("⚠️ Aviso: Osso não encontrado: ", bone_name)
...
func findhandnodes():
	...
	# Procura nos filhos do controle
	for ch in xr_controller_node.get_children():
		# O segredo está no 'true, false': isso força o Godot a olhar DENTRO de cenas instanciadas (Raio-X)
		var lskel = ch.find_child("Skeleton3D", true, false)
		
		if lskel:
			#print("✅ Skeleton3D encontrado em '", ch.name, "' com ", lskel.get_bone_count(), " ossos!")
			handnode = ch # Define a mão encontrada
			break # Para de procurar
			
	if handnode == null:
		#print("❌ ERRO: Nenhum 'Skeleton3D' foi encontrado dentro do controle!")
		return false
		
	# Salva a referência do esqueleto
	skel = handnode.find_child("Skeleton3D", true, false)
	
	if skel == null:
		#print("❌ ERRO: Skeleton3D sumiu!")
		return false
		
	# Procura a AnimationTree (também usando Raio-X)
	handanimationtree = handnode.find_child("AnimationTree", true, false)
